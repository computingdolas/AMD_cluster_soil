...
while (scalar(@ARGV) > 0) {
    ...
    {
        die "Please choose one of '-cuda', '-opencl' or '-hip' to set the operating mode.\n"
          if ($mode ne "");
        $mode = "cuda";
    }
    elsif ($arg eq "-opencl")
...
if ( ! (( -f "$bindir/Serial/OpenCL/Sort" && -x "$bindir/Serial/OpenCL/Sort" ) ||
        ( -f "$bindir/Serial/HIP/MaxFlops" && -x "$bindir/Serial/HIP/MaxFlops" ) ||
        ( -f "$bindir/Serial/HIP/DeviceMemory" && -x "$bindir/Serial/HIP/DeviceMemory" ) ||
        ( -f "$bindir/Serial/HIP/BusSpeedDownload" && -x "$bindir/Serial/HIP/BusSpeedDownload" ) ||
...
if ($mode eq "")
{
    die "Please choose either -cuda, -opencl or '-hip'.\n";
}
