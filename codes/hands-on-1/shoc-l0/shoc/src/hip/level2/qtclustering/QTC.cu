...
void QTC(const string& name, ResultDatabase &resultDB, OptionParser& op, int matrix_type){
    ...
    hipLaunchKernelGGL((compute_degrees), dim3(grid2D(thread_block_count)), dim3(tpb), 0, 0, (int *)indr_mtrx, (int *)degrees, point_count, max_degree);
    ...
    do{
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////
        ///////// -----------------               Main kernel                ----------------- /////////
        hipLaunchKernelGGL((QTC_device), dim3(grid), dim3(tpb), 0, 0, (float*)distance_matrix, (char *)Ai_mask, (char *)clustered_pnts_mask,
        ...
        if( thread_block_count > 1 ){
            ...
            hipLaunchKernelGGL((reduce_card_device), dim3(grid2D(1)), dim3(1), 0, 0, (int *)cardnl, thread_block_count);
            hipDeviceSynchronize();
            CHECK_CUDA_ERROR();
        }
        ...
        hipLaunchKernelGGL((trim_ungrouped_pnts_indr_array), dim3(grid2D(1)), dim3(tpb), 0, 0, winner_index, (int*)ungrpd_pnts_indr, (float*)distance_matrix,
        ...
        hipLaunchKernelGGL((update_clustered_pnts_mask), dim3(grid2D(1)), dim3(tpb), 0, 0, (char *)clustered_pnts_mask, (char *)Ai_mask, max_point_count);
        hipDeviceSynchronize();
        CHECK_CUDA_ERROR();
