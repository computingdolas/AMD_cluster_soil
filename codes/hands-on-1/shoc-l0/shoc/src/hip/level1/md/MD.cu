...
void runTest(const string& testName, ResultDatabase& resultDB, OptionParser& op)
{
    ...
    hipLaunchKernelGGL((compute_lj_force<T, forceVecType, posVecType, useTexture, texReader>), dim3(gridSize), dim3(blockSize), 0, 0, d_force, d_position, maxNeighbors, d_neighborList,
                     cutsq, lj1, lj2, nAtom);
    ...
    for (int i = 0; i < passes; i++)
    {
        ...
        {
            hipLaunchKernelGGL((compute_lj_force<T, forceVecType, posVecType, useTexture, texReader>), dim3(gridSize), dim3(blockSize), 0, 0, d_force, d_position, maxNeighbors, d_neighborList, cutsq,
                 lj1, lj2, nAtom);
        }
