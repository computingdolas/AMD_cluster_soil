        ...
        *flag=1;
        ...
        while (*flag)
        {
            *flag=0;
            ...
            hipEventRecord( start_cuda_event,0);
            hipLaunchKernelGGL((BFS_kernel_warp), dim3(numBlocks), dim3(devProp.maxThreadsPerBlock), 0, 0, d_costArray,d_edgeArray,d_edgeArrayAux, W_SZ, CHUNK_SZ, numVerts,
                iters,d_flag);
            CHECK_CUDA_ERROR();
            hipEventRecord(stop_cuda_event,0);
            hipEventSynchronize(stop_cuda_event);
...
void RunTest2(ResultDatabase &resultDB, OptionParser &op, Graph *G)
{
    ...
    for (int j=0;j<passes;j++)
    {
        ...
        while(frontier_length>0)
        {
            ...
            hipLaunchKernelGGL((Reset_kernel_parameters), dim3(1), dim3(1), 0, 0, d_frontier_length);
            CHECK_CUDA_ERROR();

            //kernel for frontier length within one block
            if(frontier_length<devProp.maxThreadsPerBlock)
            {
                hipLaunchKernelGGL((BFS_kernel_one_block_spill), dim3(1), dim3(devProp.maxThreadsPerBlock), shared_mem1, 0, d_frontier,frontier_length,d_costArray,d_visited,
            ...
            {
                hipLaunchKernelGGL((BFS_kernel_SM_block_spill), dim3(devProp.multiProcessorCount), dim3(devProp.maxThreadsPerBlock), shared_mem2, 0, d_frontier,d_frontier2,frontier_length,d_costArray,
            ...
            {
                hipLaunchKernelGGL((BFS_kernel_multi_block_spill), dim3(numBlocks), dim3(devProp.maxThreadsPerBlock), shared_mem2, 0, d_frontier,d_frontier2,frontier_length,d_costArray,
                 d_visited,d_edgeArray,d_edgeArrayAux,numVerts,
                 numEdges,d_frontier_length,q_size2);
                CHECK_CUDA_ERROR();

                hipLaunchKernelGGL((Frontier_copy), dim3(numBlocks), dim3(devProp.maxThreadsPerBlock), 0, 0, 
