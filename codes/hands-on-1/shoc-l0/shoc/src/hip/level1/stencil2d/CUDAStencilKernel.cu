...
struct SharedMemory<float>
{
    ...
    {
        HIP_DYNAMIC_SHARED( float, sh_float)
        // printf( "sh_float=%p\n", sh_float );
        return sh_float;
    }
};
...
struct SharedMemory<double>
{
    ...
    {
        HIP_DYNAMIC_SHARED( double, sh_double)
        // printf( "sh_double=%p\n", sh_double );
        return sh_double;
    }
};
...
CUDAStencil<T>::operator()( Matrix2D<T>& mtx, unsigned int nIters )
{
    ...
    for( unsigned int iter = 0; iter < nIters; iter++ )
    {
        ...
        hipLaunchKernelGGL((StencilKernel), dim3(dimGrid), dim3(dimBlock), localDataSize, 0,  currData,
            newData,
            mtx.GetPad(),
            lRows,
            this->wCenter,
