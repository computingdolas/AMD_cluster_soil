                     ...
                     int size )
{
    // In CUDA 4.0 we will be able to remove this level of indirection
    // if we use the cuConfigureCall and cuLaunchKernel functions.
    hipLaunchKernelGGL((scan_single_block<T,256>), dim3(num_blocks), dim3(num_threads), smem_size, 0, d_block_sums, size);
}
                        ...
                        int size )
{
    // In CUDA 4.0 we will be able to remove this level of indirection
    // if we use the cuConfigureCall and cuLaunchKernel functions.
    hipLaunchKernelGGL((bottom_scan<T, vecT, blockSize>), dim3(num_blocks), dim3(num_threads), smem_size, 0, g_idata, g_odata,
        d_block_sums, size);
}
