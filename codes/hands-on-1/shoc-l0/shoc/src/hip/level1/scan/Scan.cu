...
void RunTest(string testName, ResultDatabase &resultDB, OptionParser &op)
{
    ...
    for (int k = 0; k < passes; k++)
    {
        ...
        for (int j = 0; j < iters; j++)
        {
            ...
            hipLaunchKernelGGL((reduce<T, 256>), dim3(num_blocks), dim3(num_threads), smem_size, 0, d_idata, d_block_sums, size);

            // Next, a top-level exclusive scan is performed on the array
            // of block sums
            hipLaunchKernelGGL((scan_single_block<T, 256>), dim3(1), dim3(num_threads), smem_size*2, 0, d_block_sums, num_blocks);

            // Finally, a bottom-level scan is performed by each block
            // that is seeded with the scanned value in block sums
            hipLaunchKernelGGL((bottom_scan<T, vecT, 256>), dim3(num_blocks), dim3(num_threads), 2*smem_size, 0, d_idata, d_odata, d_block_sums, size);
        }
